...
public class DatabaseManager {
	public static void init(DataSource dataSource) {
		try (Connection connection = dataSource.getConnection(); PreparedStatement ps = connection
			...
			log.error("Droped down at DatabaseManager public static void init(DataSource " + "dataSource because of \n" + e
					.getMessage());
		}
	}
	...
	private static String getInitQuery() {
		try (InputStream goodssql = DatabaseManager.class
			...
			log.error("Droped down at private static String getInitQuery() in DatabaseManager because of \n" + e
					.getMessage());
		}
		return null;
	}
	...
	public static void drop(DataSource dataSource) {
		try (Connection connection = dataSource.getConnection(); PreparedStatement ps = connection
			...
			log.error("Droped down at public static void drop(DataSource dataSource) in DatabaseManager because of \n" + e
					.getMessage());
		}
	}
